import { Button, Col, Container, Form, InputGroup, Row, Dropdown, Image, Spinner, Modal, Card, OverlayTrigger, Tooltip } from "react-bootstrap";
...
function HomePage() {
    ...
    const selectWallet = async (id: number) => {
        ...
        const body = {
            walletAddress: (selectwallet[0].address).replace(/^"|"$/g, '')
        }
        console.log(body)
    ...
    const startTrading = async () => {
        ...
        if (balance < buyAmount || balance < sellAmount) {
                    ...
                    <Row>
                        ...
                        <Col sm={12} lg={6} className="mb-5">
                            <MainCard title="Trading Info">
                                <Row>
                                    <Col xs={12} lg={6} className="mb-3 d-flex align-items-center">
                                        {walletList.length < 1 ? (
                                            <Button className="mb-3 Main-btn rounded-0 fw-bold" onClick={createWallet}>CREATE WALLET</Button>
                                        ) : (
                                            <Dropdown role="menuitemradio">
                                                <Dropdown.Toggle variant="success" id="dropdown-basic" className="Main-btn rounded-0 fw-bold mb-3" disabled={((currentTrading?.status === false || currentTrading === null) || currentTrading === null) ? false : true}>
                                                    {selectedWallet === null ? "Select Wallet" : ((JSON.parse(selectedWallet?.address)).slice(0, 6) + "..." + (JSON.parse(selectedWallet?.address)).slice(-4))}
                                                </Dropdown.Toggle>
                                                <Dropdown.Menu>
                                                    {walletList?.map((item, index) => (
                                                        <Dropdown.Item as="li" key={index}>
                                                            <Form.Check
                                                                inline
                                                                label={(JSON.parse(item.address)).slice(0, 6) + "..." + (JSON.parse(item.address)).slice(-4)}
                                                                name={`group`}
                                                                type='radio'
                                        ...
                                        >
                                            <Button className="mb-3 ms-3 text-center rounded-0 py-1 px-2 bg-transparent border-1 Main-btn" onClick={() => { copy(selectedWallet?.privateKey as string as string); toast.info("Private key Copied!"); console.log(base58.encode(Object.values(JSON.parse(selectedWallet?.privateKey as string)))) }}><Image src={KeyIcon} width={20} alt="clipboard" className="mb-1" /></Button>
                                        </OverlayTrigger>
                                    </Col>
                                    <Col xs={12} lg={6} className="mb-3 text-start fw-bold fs-6 d-flex align-items-center justify-content-start justify-content-lg-end">
                                        Balance: {balance} ETH
                                    </Col>
                                    ...
                                    <Col sm={6} lg={6} className="mb-3">
                                        <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
                                            <Form.Label>Buy Amount <small>(ETH)</small></Form.Label>
                                    ...
                                    <Col sm={6} lg={6} className="mb-3">
                                        <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
                                            <Form.Label>Sell Amount <small>(ETH)</small></Form.Label>
